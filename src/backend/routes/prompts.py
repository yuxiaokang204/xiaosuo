"""
Prompt 管理路由 - Agent Prompt CRUD 和激活管理
"""
...
async def list_prompts(
    ...
    async with AsyncSessionLocal() as session:
        ...
        return {
            "prompts": [
                {
                    "id": p.id, "novel_id": p.novel_id, "agent_type": p.agent_type,
                    "depth_level": p.depth_level, "prompt_type": p.prompt_type,
                    "title": p.title, "content": p.content,
                    "quality_score": p.quality_score, "usage_count": p.usage_count,
    ...
    async with AsyncSessionLocal() as session:
        ...
        if req.is_active:
            await session.execute(
                ...
                .where(and_(
                    AgentPromptDB.agent_type == req.agent_type,
    ...
    async with AsyncSessionLocal() as session:
        ...
        if req.is_active:
            await session.execute(
                update(AgentPromptDB)
                .where(and_(
                    AgentPromptDB.agent_type == p.agent_type,
...
async def batch_delete_prompts(ids: list[str]):
    """批量删除 prompts"""
    from ..db.database import AsyncSessionLocal
    from ..db.models import AgentPromptDB
...
@router.get("/prompts/active/{agent_type}")
async def get_active_prompt(agent_type: str, depth_level: int = 1,
                            prompt_type: str = "system", novel_id: Optional[str] = None):
    """获取指定 agent 当前激活的 prompt"""
...
@router.post("/prompts/save-from-run")
async def save_prompts_from_run(req: Dict[str, Any]):
    """将某次 loop 运行中的 prompt 批量保存为模板"""
...
async def clear_prompt_cache():
    ...
    clear_cache()
    return {"success": True, "message": "Prompt 缓存已清除"}


@router.post("/prompts/seed-defaults")
async def seed_default_prompts():
    """初始化6个Agent的默认Skill Prompt模板到数据库（每个Agent一条）"""
    from ..db.models import AgentPromptDB
    from ..db.database import AsyncSessionLocal
